class MyProductsController < ApplicationController
  ...
  def create
    ...
    if @product.save
      redirect_to my_product_path(@product), notice: "Product was successfully created."
    else
      @categories = ShopCategory.all.order(:name)
      render :new
    end
  ...
  def update
    if @product.update(product_params)
      redirect_to my_product_path(@product), notice: "Product was successfully updated."
    else
      @categories = ShopCategory.all.order(:name)
      render :edit
    end
  ...
  def destroy
    if @product.order_items.exists?
      redirect_to my_products_path, alert: "Cannot delete product with associated orders."
    else
      @product.destroy
    ...
    if params[:csv_file].present?
      begin
        ...
        if @failure_count.zero?
          redirect_to my_products_path, notice: "Successfully imported #{@success_count} products."
        else
  ...
  def bulk_edit
    ...
    if @products.empty?
      redirect_to my_products_path, alert: "No products selected for bulk editing."
    end
  end
  ...
  def bulk_update
    ...
    if @products.empty?
      redirect_to my_products_path, alert: "No products selected for bulk updating."
      return
    end
    ...
    if params[:price_change_type] == 'percentage' && params[:price_change_value].present?
      ...
      if update_params.present?
        ...
        redirect_to my_products_path, alert: "No update parameters provided."
        return
      end
    end

    if failure_count.zero?
      redirect_to my_products_path, notice: "Successfully updated #{success_count} products."
    else
      redirect_to my_products_path, alert: "Updated #{success_count} products with #{failure_count} failures."
    end
  end
  ...
  def require_seller
    unless current_user.seller?
      redirect_to root_path, alert: "You must be a seller to access this page."
    end
  end
  ...
  def set_product
    ...
    redirect_to my_products_path, alert: "Product not found or you don't have permission to access it."
  end

  def product_params
    params.require(:product).permit(
      :name, :description, :price, :original_price, :stock_quantity, :sku,
      :shop_category_id, :status, :brand, :product_type, :download_limit,
      :weight, :weight_unit, :length, :width, :height, :dimension_unit,
      specifications: {}, tags: [], images: [], digital_file: []
    )
  end
end
