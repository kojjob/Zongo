...
(function() {
  ...
  // Check if we have a user setting in the HTML (data attribute)
  const userSettingElement = document.querySelector('[data-theme-preference]');
  const userSetting = userSettingElement ? userSettingElement.dataset.themePreference : null;

  // If we have a user setting, use it to set localStorage
  if (userSetting) {
    if (userSetting === 'light' || userSetting === 'dark') {
      localStorage.setItem('theme', userSetting);
    } else if (userSetting === 'auto') {
      localStorage.removeItem('theme');
    }
    console.log(`Theme Initializer: Applied user setting: ${userSetting}`);
  }

  // Get the updated theme from localStorage
  const finalTheme = localStorage.getItem('theme');

  // Determine if we should use dark mode
  const shouldBeDark = finalTheme === 'dark' ||
                       (finalTheme !== 'light' && systemPrefersDark);
