...
export default class extends Controller {
  ...
  updateTotal() {
    ...
    // Find and update the fee display if it exists
    const feeDisplay = document.querySelector('[data-transaction-form-target="summaryFee"]')
    if (feeDisplay) {
      feeDisplay.textContent = `${currencySymbol}${fee.toFixed(2)}`
    }
   ...
   */
  calculateFee(amount) {
    // Get transaction type from the URL or data attribute
    const urlParams = new URLSearchParams(window.location.search)
    const transactionType = urlParams.get('type') || this.element.dataset.transactionType || 'deposit'

    // Different fee structures based on transaction type
    switch(transactionType) {
      case 'deposit':
        // Deposits have a 1% fee with a minimum of 1 and maximum of 10
        return Math.min(Math.max(amount * 0.01, 1), 10)

      case 'withdrawal':
        // Withdrawals have a 1.5% fee with a minimum of 2
        return Math.max(amount * 0.015, 2)

      case 'transfer':
        // Transfers have a flat fee of 1 for amounts under 100, otherwise 0.5%
        return amount < 100 ? 1 : amount * 0.005
