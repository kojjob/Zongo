...
export default class extends Controller {
  ...
  setupAccessibility() {
    if (this.hasTabsListTarget) {
      ...
      // Set up ARIA attributes for each tab and panel
      this.tabTargets.forEach((tab, index) => {
        const tabId = tab.getAttribute('data-tab-id')
        const panel = this.tabPanelTargets.find(panel => panel.getAttribute('data-tab-id') === tabId)
        ...
        // Set ARIA attributes for tab
        tab.setAttribute('role', 'tab')
        tab.setAttribute('aria-selected', tab === this.activeTab)
        tab.setAttribute('aria-controls', panel ? panel.id : '')
        tab.setAttribute('tabindex', tab === this.activeTab ? '0' : '-1')
  ...
  keydown(event) {
    ...
    switch (event.key) {
      ...
      case 'ArrowDown':
        event.preventDefault()
        nextIndex = this.getNextTabIndex(currentIndex)
        break
      case 'ArrowLeft':
      case 'ArrowUp':
        event.preventDefault()
        nextIndex = this.getPreviousTabIndex(currentIndex)
        break
      case 'Home':
        event.preventDefault()
        nextIndex = 0
    ...
    // If we have a next index, focus that tab
    if (nextIndex !== null) {
  ...
  /**
   * Get the next tab index (with wrapping)
   */
  getNextTabIndex(currentIndex) {
    return (currentIndex + 1) % this.tabTargets.length
  }

  /**
   * Get the previous tab index (with wrapping)
   */
  getPreviousTabIndex(currentIndex) {
  ...
  announceTabChange(tab) {
    ...
    let liveRegion = document.getElementById('tabs-live-region')
    if (!liveRegion) {
      liveRegion = document.createElement('div')
      liveRegion.id = 'tabs-live-region'
    ...
    liveRegion.textContent = `Tab ${tabLabel} selected`
