module PaymentProcessors
  # Processor for MTN Mobile Money transactions
  class MtnMobileMoneyProcessor < BaseProcessor
    ...
    def verify_deposit(amount:, currency:, reference:, metadata: {}, verification_data: {})
      ...
      if result[:success]
        success_response("MTN Mobile Money deposit verified successfully", reference, {
          provider_data: {
            transaction_id: result[:mtn_transaction_id],
    ...
    # Simulate a response from MTN Mobile Money
    ...
    # @return [Hash] Simulated MTN response
    def simulate_mtn_response(operation:, amount:, currency:, reference:, phone_number: nil, merchant_code: nil)
      ...
      if rand(100) < 5
        ...
        error_messages = {
          ...
          amount_mismatch: "Amount does not match MTN records",
          expired_reference: "Reference has expired",
          insufficient_funds: "Insufficient funds in MTN account",
          invalid_phone_number: "Invalid phone number format",
          service_unavailable: "MTN service temporarily unavailable",
